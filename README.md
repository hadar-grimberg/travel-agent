...
| `OPENTRIPMAP_API_KEY` | Optional — POI discovery within drive radius |
...
**`feedback` — relay one user reply:**

```json
{ "action": "feedback", "session_id": "<from start>", "feedback": "I love the kaiseki idea, more like that" }
```

Reply keywords the graph reacts to:
- `"approve"`, `"next"`, `"looks good"`, `"move on"` → approve current category, advance to the next
- `"create itinerary"`, `"finalize"`, `"book it"`, `"ready to book"` → skip remaining categories, build the itinerary now
- `"exit"`, `"quit"`, `"stop"` → end the session without an itinerary
- anything else → treated as picks/preferences; the agent recommends more similar activities in the same category
...
```json
{
  ...
  "recommendations": {
    ...
    "question": "Pick your favorites, ask for more like these, or say 'approve' to move on.",
    "categories_remaining": ["culture", "nature"]
  },
  "prompt": "Pick your favorite 'food' activities, ask for more like these, or say 'approve' to move to the next category:"
}
```
...
```json
{
  ...
  "user_preferences": ["[food] I love the kaiseki idea..."]
}
```
...
flowchart TD
    ...
    user -->|picks / more like these| recommend
    user -->|approve, or 6-round limit| advance[advance_category]
    advance -->|categories left| recommend
    advance -->|all done| synthesize[synthesize_itinerary]
    user -->|create itinerary / finalize| synthesize
    user -->|exit| END
...
| `discover_nearby_activities_tool` | POIs within 4h drive (OpenTripMap or fallback) |
